...
// Declare the one entry point we need instead; lodepng.c is compiled as C
// and uses plain malloc, so the caller frees with free().
#include <lvgl.h>
...
bool ensureInit()
{
    ...
    for (uint8_t count = kMaxSlots; count >= 4u; count >>= 1u) {
        s_pixels = static_cast<uint8_t *>(heap_caps_malloc(
            static_cast<size_t>(count) * kTileBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
        if (s_pixels != nullptr) {
            s_slot_count = count;
            break;
        }
...
void tilePath(const TileKey &key, char *out, size_t out_size)
{
    ...
    snprintf(out, out_size, "%s/tiles/%u/%ld/%ld.jpg", mnt != nullptr ? mnt : "",
             static_cast<unsigned>(key.z), static_cast<long>(key.x), static_cast<long>(key.y));
}
...
bool httpDownload(const TileKey &key, uint8_t *buf, size_t cap, size_t *out_size)
{
    ...
    // OSM's tile-usage policy rejects the stock ESP HTTP user agent.
    (void)esp_http_client_set_header(http, "User-Agent", "NRL-ESP32 APRS map (offline tile cache)");
    bool ok = false;
...
bool decodePngToTile(const uint8_t *data, size_t size, uint8_t *out)
{
    ...
    if (err != 0u || rgba == nullptr || w == 0u || h == 0u) {
...
void writeThroughSd(const TileKey &key, const uint8_t *rgb565)
{
    ...
    const char *mnt = STORAGE_SdMountPoint();
    char dir[96];
    snprintf(dir, sizeof(dir), "%s/tiles", mnt);
    (void)mkdir(dir, 0775);
    snprintf(dir, sizeof(dir), "%s/tiles/%u", mnt, static_cast<unsigned>(key.z));
    (void)mkdir(dir, 0775);
    snprintf(dir, sizeof(dir), "%s/tiles/%u/%ld", mnt, static_cast<unsigned>(key.z),
             static_cast<long>(key.x));
    (void)mkdir(dir, 0775);
...
bool loadTile(const TileKey &key, uint8_t *out)
{
    uint8_t *buf = static_cast<uint8_t *>(
        heap_caps_malloc(kDownloadCap, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (buf == nullptr) {
        return false;
    }
    size_t size = 0u;
...
void mapTilesTask(void *)
{
    for (;;) {
        ...
        const bool ok = loadTile(key, pixels);
...
void MAP_TILES_Request(uint8_t z, int32_t x, int32_t y)
{
    ...
    if (slot >= 0) {
        ...
        const bool cool_down = state == SlotState::Failed &&
                               (millis() - s_slots[slot].fail_ms) < kFailRetryMs;
